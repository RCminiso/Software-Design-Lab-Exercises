...
def main():
    ...
    if choice == "Create":
        ...
        if st.button("Add Task"):
            db_fxns.add_data(task, task_status, task_due_date)
        ...
        list_of_tasks = [i[0] for i in db_fxns.view_all_task_names()]
        selected_task = st.selectbox("Task", list_of_tasks)
        task_result = db_fxns.get_task(selected_task)
        ...
        if task_result:
            ...
            if st.button("Update Task"):
                db_fxns.edit_task_data(new_task, new_task_status, new_task_due_date, task, task_status, task_due_date)
        ...
        unique_list = [i[0] for i in db_fxns.view_all_task_names()]
        delete_by_task_name = st.selectbox("Select Task", unique_list)
        if st.button("Delete"):
            db_fxns.delete_data(delete_by_task_name)
